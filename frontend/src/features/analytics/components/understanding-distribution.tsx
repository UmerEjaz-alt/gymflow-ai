...
export function UnderstandingDistribution({ metrics }: { metrics: DashboardMetrics }) {
  ...
  return (
    <div className="bg-card border-border flex h-full flex-col rounded-xl border p-6">
      <h2 className="mb-4 text-lg font-semibold tracking-tight">Conversation Stage Analytics</h2>
      {metrics.understandingDistribution.length === 0 ? (
        <div className="text-muted-foreground text-sm">No understanding data available yet.</div>
      ) : (
        <div className="space-y-4">
          {metrics.understandingDistribution.map(({ stage, count }) => {
            const percentage =
              totalWithUnderstanding > 0 ? Math.round((count / totalWithUnderstanding) * 100) : 0;
            return (
