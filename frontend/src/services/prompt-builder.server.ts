  ...
  if (isAutomation) {
    return {
      ...
      content: [
        ...
        "This turn is an AUTOMATED OUTBOUND MESSAGE initiated by GymFlow — the customer did NOT just send a new message.",
        "",
...
export function buildSystemPrompt(
  ...
  const branchRule = isUnresolvedMultiBranch
    ...
    : "Primary-branch sections apply only to the primary branch. For another named branch, use only its Other Branch Information; exploratory questions never change selected_branch_id. Set selected_branch_id only for an unresolved selection or an explicit switch/commitment.";
  ...
  return {
    label: "system",
    content: [
      "You are GymFlow's professional gym receptionist.",
      `Business context: ${identity}.`,
      mode,
      "Use English or Roman Urdu naturally to match the customer. Use only supplied knowledge; never invent facts, policies, prices, media, exceptions, or contact details. Never expose internal IDs.",
...
export function buildKnowledgeSummary(knowledge: KnowledgeContext): PromptSection {
  ...
  if (branch) {
    ...
    if (branch.opening_hours && (knowledge.needs.all || knowledge.needs.openingHours)) {
      lines.push("");
    ...
    if (branch.faqs && branch.faqs.length > 0) {
      lines.push("");
      lines.push(
  ...
  const needsBranchDirectory =
    !branch || (knowledge.crossBranchKnowledge?.length ?? 0) > 0;
  if (
    ...
    if (!branch) {
      lines.push(
        "This business has multiple locations. The customer has NOT selected a branch yet (branch is UNRESOLVED). For generic questions (e.g. 'what are your fees?'), ask which branch they are interested in by name. If details for a specific branch are loaded below under ## Other Branch Information, use them to answer explicit questions or provide details immediately upon branch selection.",
      );
      for (const b of knowledge.allBranches) {
        const parts = [b.branch_name];
  ...
  // ── Facilities ────────────────────────────────────────────────────────────
  if (knowledge.facilities && knowledge.facilities.length > 0) {
    lines.push("");
  ...
  } else if (knowledge.facilities !== null) {
    lines.push("");
    lines.push("## Facilities & Amenities");
  ...
  // ── Media assets ──────────────────────────────────────────────────────────
  if (knowledge.media && knowledge.media.length > 0) {
    lines.push("");
  ...
  // ── Membership packages ──────────────────────────────────────────────────
  if (knowledge.packages && knowledge.packages.length > 0) {
    lines.push("");
    if (branch) {
  ...
  } else if (knowledge.packages !== null) {
    lines.push("");
    lines.push("## Membership Packages");
  ...
  // ── Trainers ─────────────────────────────────────────────────────────────
  if (knowledge.trainers && knowledge.trainers.length > 0) {
    lines.push("");
    lines.push("## Trainers");
  ...
  } else if (knowledge.trainers !== null) {
    lines.push("");
    lines.push("## Trainers");
